...
class WrongCarFileException(Exception):
    def __init__(self, file_type, expected_file_types):
        ...
        Exception.__init__(self,
                           'Open method does not support CAR file type: %s. Instead expect following CAR file types: %s' % (
                               str(file_type), str(expected_file_types)))


class BrokenCborFileException(Exception):
    def __init__(self):
        Exception.__init__(self,
                           'Corrupt, incomplete, or otherwise broken CBOR file. Please re-download or contact the '
                           'organizers or use appropriate reader to open this file.')
...
class Page(object):
    ...
    def get_text(self):
        """Include all visible text below this elements. Includes Captions of images, but no headings and no
        infoboxes. See `get_text_with_headings` for a version that includes headings. """
        return '\n'.join(skel.get_text() for skel in self.skeleton)

    def get_text_with_headings(self, include_heading=False):
        """Include all visible text below this elements. While the heading of this element is excluded, headings of
        subsections will be included. Captions of images are excluded. """
...
class PageMetadata(object):
    ...
    def __str__(self):
        ...
        catStr = ("" if self.redirectNames is None else (
                " categories = " + ", ".join([name for name in (self.categoryNames or [])])))
        ...
        pageTagsStr = (
            "" if self.pageTags is None else (" pageTags = " + ", ".join([name for name in (self.pageTags or [])])))
        return "%s \n%s \n%s \n%s \n%s\n%s \n%s \n%s \n" % (
    ...
    def from_cbor(cbor):
        ...
        return PageMetadata(redirectNames, disambiguationNames, disambiguationIds, categoryNames, categoryIds,
                            inlinkIds, inlinkAnchors, wikiDataQid, siteId, pageTags)
    ...
    * :class:`Image`
    ...
    def from_cbor(cbor):
        ...
        if tag == 0:  # section
            ...
            cbor_entries = cbor[2]
            entries = [(kv[0], PageSkeleton.from_cbor(kv[1][0])) for kv in cbor_entries if
                       kv[1] and kv[1][0]]  # if no value is defined kv[1] will be null.
